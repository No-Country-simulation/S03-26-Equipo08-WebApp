...
import { User, Mail, Lock, ArrowRight, ChevronLeft, ShieldCheck, Building2 } from "lucide-react";
...
// itemVariants removed as it is unused
...
export default function RegisterPage() {
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans selection:bg-indigo-100 selection:text-indigo-900">
      ...
      {/* Background Decor - Matching Landing Spotlight */}
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-center text-center mb-10">
            ...
            <p className="text-gray-400 font-medium text-lg">Únete a la infraestructura de confianza para marcas.</p>
          </div>
          ...
          <form className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="relative group">
                 <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-300 group-focus-within:text-[#6366f1] transition-colors" />
                 <input 
                   type="text" 
                   placeholder="Hernán García" 
                   className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-50 focus:border-indigo-100 outline-none transition-all font-semibold text-sm text-gray-900 placeholder:text-gray-300 shadow-sm"
                 />
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">Compañía / Marca</label>
              <div className="relative group">
                 <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-300 group-focus-within:text-[#6366f1] transition-colors" />
                 <input 
                   type="text" 
                   placeholder="Hub S.A." 
                   className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-transparent rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-50 focus:border-indigo-100 outline-none transition-all font-semibold text-sm text-gray-900 placeholder:text-gray-300 shadow-sm"
            ...
            <div className="md:col-span-2 pt-4">
              <button className="w-full bg-gray-900 text-white font-black py-6 rounded-[2rem] shadow-2xl hover:bg-black transition-all active:scale-[0.98] flex items-center justify-center gap-3 group relative overflow-hidden">
                 <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700" />
                 Finalizar Registro Hub
      ...
      >
         ...
         <span className="text-[10px] font-bold uppercase tracking-[0.25em]">GDPR Compliance • Testimonial Hub</span>
      </motion.div>
    </div>
  );
}
